...
const BankPage = ({ bank, cards }) => {
  ...
  const { trackComponentError, trackComponentInteraction } = useComponentAnalytics('BankPage');
  ...
  }, [bank, cards, trackFeatureUsage, trackJourneyStep, trackEvent, recordCustomMetric, user, isMobile, isTablet]);

  // Track card images loading
  useEffect(() => {
    if (cardImagesData?.length > 0) {
      const bankCardsWithImages = getCardsWithImages();
      const imagesLoadedCount = bankCardsWithImages.filter(card => card.image).length;
