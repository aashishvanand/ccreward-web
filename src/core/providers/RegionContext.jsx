...
export function RegionProvider({ children }) {
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (e) => {
      if (e.key === 'app-region' && e.newValue && e.newValue !== region) {
        console.log(`💼 Region changed in storage: ${e.oldValue} -> ${e.newValue}`);
  ...
  useEffect(() => {
    if (!isLoading) {
      ...
      if (currentStoredRegion !== region) {
        console.log(`💼 Synchronizing localStorage with current region: ${region}`);
  ...
  const updateRegion = (newRegion) => {
    ...
    // Store region in lowercase in localStorage for consistency with database
    localStorage.setItem('app-region', newRegion.toLowerCase());

    
    // Update state - this will trigger the effect that updates localStorage
